...
class MyWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        for gpu in gpus:
            show_info = show_info_str + gpu.Name + '\n'
        show_info_str = '*' * 40 + '\n'
    ...
    def update_model(self, process_list):
        ...
        for process in process_list:
            ...
            item4 = QStandardItem(str_creation_date[:4]+'-'+str_creation_date[4:6]+'-'+str_creation_date[6:8]+' '
...
class WorkThread(QThread):
    ...
    def run(self):
        ...
        print('wmi load process_list used {} seconds'.format(time.time() - start_time))
...
class WorkThread2(QThread):
    ...
    def run(self):
        ...
        show_info_str = show_info_str + 'CPU：'+ processor.Name + '，\n   内核：'+ str(processor.NumberOfCores) + '，' + \
                        '逻辑处理器：'+ str(processor.NumberOfLogicalProcessors) +'，L2 Cache：'+ \
                        str(float(processor.L2CacheSize) / 1024) + 'M，L3 Cache：' + \
                        str(float(processor.L3CacheSize) / 1024) +'M\n'
        show_info_str = show_info_str + '显卡：\n'
        for gpu in gpus:
            show_info_str = show_info_str + '   ' + gpu.Name + '\n'
        show_info_str = show_info_str + '*' * 60 + '\n'

        print('wmi load system info used {} seconds'.format(time.time() - start_time))
        pythoncom.CoUninitialize()
