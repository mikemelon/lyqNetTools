from OpenSSL.crypto import (load_certificate, dump_privatekey, dump_certificate,dump_publickey, X509, X509Name, PKey)
...
def format_split_bytes(aa):
    ...
    out = format(':'.join(s.to_bytes(1, byteorder='big').hex().lower() for s in bytes.fromhex(bb)))
    return out


def format_split_int(serial_number):
    # aa = "0%x" % serial_number  # add leading 0
    aa = "0{}".format(serial_number.hex())  # add leading 0
...
def parse_cert(cert_file):
    with open(cert_file, 'rb+') as f:
        ...
        print("\n".join(pkey_lines))
        print("        X509v3 extensions:" + str(x509.get_extension_count()) + ' used')
...
if __name__ == "__main__":
    ...
    os.chdir(sys.path[0])
    parse_cert(r"c:\rsatest1\baidu1.cer")
    sys.exit(0)